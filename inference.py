...
def block_generate(data_lines, model, AR_model, task, block_size, device, max_len=200):
    src_dict = task.source_dictionary
    tgt_dict = task.target_dictionary
    encoder_state_ids = []
    ...
    for start_idx in tqdm(range(0, data_size)):
        ...
        start_pos = 0
        for step in range(0, max_len):
            start_pos, prev_output_tokens = block_forward(incremental_state, encoder_state_ids,
...
def block_forward(incremental_state, encoder_state_ids, start_pos, block_size, tgt_dict, prev_output_tokens,
                  encoder_out, AR_encoder_out, model, AR_model, max_len=200):
    ...
    _AR_tokens = AR_forward_decoder(AR_model,
                                    cur_span_input_tokens,
                                    AR_encoder_out,
                                    incremental_state,
                                    parallel_forward_start_pos=start_pos)

    AR_verify_tokens = _AR_tokens.tolist()

    bifurcation = block_size
    for i, (token, AR_token) in enumerate(zip(prev_output_tokens[start_pos:], AR_verify_tokens[:-1])):
        if token != AR_token:
            bifurcation = i
            break
    next_output_tokens = prev_output_tokens[:start_pos] + AR_verify_tokens[:bifurcation + 1] + [
        tgt_dict.unk()] * block_size

    find_eos = False
    for i, o in enumerate(AR_verify_tokens[:bifurcation + 1]):
    ...
    return start_pos, next_output_tokens
...
if __name__ == '__main__':
    ...
    parser.add_argument('--strategy', type=str, default='fairseq',
                        help='decoding strategy, choose from: fairseq, AR, block')
    ...
    if cmd_args.strategy == 'AR':
        logger.info("Decoding Strategy: AR")
        remove_bpe_results, delta = baseline_generate(bpe_sents, AR_model, _AR_model_task, device)
        logger.info(f'Baseline generate: {delta}')
    elif cmd_args.strategy == 'block':
        logger.info("Decoding Strategy: Block")
        remove_bpe_results, delta = block_generate(bpe_sents, model, AR_model, task, cmd_args.block_size, device)
